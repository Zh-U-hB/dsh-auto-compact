 ...
 */
function estimateFallbackEnvelope(session) {
  let header
  let headerSeq = -1
  for (const event of Object.values(session?.events ?? {})) {
    if (event?.type !== 'request/header') continue
    const seq = typeof event.seq === 'number' ? event.seq : headerSeq + 1
    if (seq >= headerSeq) {
      headerSeq = seq
      header = event.data
    }
  ...
  // Mirrors @deepseek-ai/dsh-token-meter's fixed-density envelope heuristic:
  // 4 chars/token plus per-block structural overhead.
  let tokens = 0
  if (typeof header.system === 'string') tokens += Math.ceil(header.system.length / 4) + 4
  if (Array.isArray(header.tools) && header.tools.length > 0) {
    tokens += Math.ceil(JSON.stringify(header.tools).length / 4) + 4
  }
  return tokens
}
...
function estimateSurfaceMeasurement(ctx, session) {
  ...
  const envelopeTokens = estimateFallbackEnvelope(session)
  return {
    baseline: { kind: 'estimated', tokens: surfaceTokens + envelopeTokens },
    envelopeTokens,
    surfaceTokens,
...
async function enforceThreshold(ctx, compaction, agent, config, signal, warned, preMeasurement) {
  for (let attempt = 0; attempt < config.maxCompactions; attempt += 1) {
    if (signal.aborted) return
    const measurement = attempt === 0 && preMeasurement !== undefined
      ? preMeasurement
...
/**
 * Idle/resume compaction: when an agent is announced while idle (a session is
 * newly opened or resumed) and its measured context is already at/above the
 * threshold, compact through the same backend's `compactNow` maintenance path
 * — the exact path the built-in `/compact` command uses. This is what makes a
 * reopened large conversation visibly compact without requiring the user to
 * send another message first.
 */
async function enforceIdleThreshold(ctx, compaction, agent, config, warned) {
  if (agent?.status !== 'idle' && agent?.status !== undefined) return
  const measurement = ctx.tokenMeter.measure(agent.session)
  const total = typeof measurement?.totalTokens === 'number' ? measurement.totalTokens : 0
  if (total < config.thresholdTokens) return
  ...
  if (range === null) {
    if (!warned.noRange.has(agent.session)) {
      warned.noRange.add(agent.session)
      ctx.logger.warn(
        `dsh-auto-compact: idle context at ${formatCount(total)} tokens is at/above the ${formatCount(config.thresholdTokens)} threshold, ` +
        `but no tool-pair-balanced older span is compactable`,
      )
    }
    return
  }
  ...
  ctx.logger.info(
    `dsh-auto-compact: idle context at ${formatCount(total)} tokens reached the ${formatCount(config.thresholdTokens)} threshold; ` +
    `compacting surface seqs ${range.start}-${range.end}`,
  )
  ...
  if (result !== null && result !== undefined) {
    ...
    const shadowedTokens = typeof result.shadowedTokenCount === 'number' ? result.shadowedTokenCount : 0
    ctx.logger.info(
      `dsh-auto-compact: idle compaction shadowed ${String(shadowed)} history items (~${formatCount(shadowedTokens)} tokens)`,
    )
  }
}
...
const SETTINGS_HTTP_ROOT = '/dsh-auto-compact'
const SETTINGS_DIAG_PATH = '/dsh-auto-compact/diag'

function describeSessionForDiagnostics(sctx, diagnostics, sessionId) {
  const agent = sctx.agents.get(sessionId)
  if (agent === undefined) return undefined
  let measurement
  let measureError
  try {
    const value = sctx.tokenMeter.measure(agent.session)
    measurement = {
      totalTokens: value.totalTokens,
      surfaceTokens: value.surfaceTokens,
  ...
  if (measurement !== undefined) {
    try {
      range = selectCompactableRange(agent.session, sctx.tokenMeter.measure(agent.session), diagnostics.config().retainTokens)
    } catch {
      range = null
    }
  }
  ...
  return {
    sessionId,
    preset: agent.options?.agentPreset,
    config: {
      thresholdTokens: diagnostics.config().thresholdTokens,
      retainTokens: diagnostics.config().retainTokens,
      maxCompactions: diagnostics.config().maxCompactions,
    ...
    range,
    backend: {
      direct: direct !== undefined && typeof direct.compactRegion === 'function',
      fallbackMounted: diagnostics.fallbackEngines.has(agent),
    },
    decisions: diagnostics.decisions.get(sessionId) ?? [],
  }
}

function installSettingsHttp(ctx, getScope, diagnostics) {
  if (typeof ctx.inject !== 'function') return
  ctx.inject(['webServer', 'settings', 'agents', 'tokenMeter'], (sctx) => {
    sctx.effect(() => sctx.webServer.register({
      kind: 'prefix',
      path: SETTINGS_HTTP_ROOT,
      handler: async (req, res) => {
        ...
        if (pathname === SETTINGS_DIAG_PATH) {
          if (req.method !== 'GET') {
          ...
          if (typeof sessionId !== 'string' || sessionId.length === 0) {
            sendJson(res, 400, { ok: false, error: 'missing sessionId query parameter' })
            return
          }
          const diagnostic = describeSessionForDiagnostics(sctx, diagnostics, sessionId)
          if (diagnostic === undefined) {
            sendJson(res, 404, { ok: false, error: `agent "${sessionId}" is not live` })
            return
          }
          sendJson(res, 200, { ok: true, ...diagnostic })
          return
        }
        if (pathname !== SETTINGS_HTTP_PATH) {
    ...
    }), 'dsh-auto-compact: settings + diagnostics HTTP route')
  })
}
...
export function apply(ctx, rawConfig) {
  ...
  const diagnostics = {
    decisions: new Map(),
    config: () => config,
    fallbackEngines: warned.fallbackEngines,
  }
  const recordDecision = (agent, stage, data = {}) => {
    const list = diagnostics.decisions.get(agent.id) ?? []
    list.push({ stage, time: Date.now(), ...data })
    diagnostics.decisions.set(agent.id, list.slice(-20))
  }
  const measureForDecision = (agent, stage) => {
    try {
      const measurement = ctx.tokenMeter.measure(agent.session)
      const range = selectCompactableRange(agent.session, measurement, config.retainTokens)
      recordDecision(agent, stage, {
        totalTokens: measurement.totalTokens,
        surfaceTokens: measurement.surfaceTokens,
        nodes: measurement.nodes?.length ?? 0,
        range: range === null ? null : { start: range.start, end: range.end },
      })
      return measurement
    } catch (error) {
      recordDecision(agent, stage, { measureError: error instanceof Error ? error.message : String(error) })
      return undefined
    }
  }

  installSettingsHttp(ctx, () => settingsScope, diagnostics)

  installMeterFallback(ctx, warned.meterFallback)

  ctx.on('agent/pre-step', async ({ agent, signal }, next) => {
    recordDecision(agent, 'pre-step')
    const preMeasurement = measureForDecision(agent, 'pre-step-measure')
    const compaction = await resolveCompaction(ctx, agent, warned.fallbackEngines, warned.fallbackEngineClass)
    recordDecision(agent, 'pre-step-backend', {
      resolved: compaction !== undefined && typeof compaction.compactRegion === 'function',
      fallbackMounted: warned.fallbackEngines.has(agent),
    })
    ...
    try {
      await enforceThreshold(ctx, compaction, agent, config, signal, warned, preMeasurement)
    } catch (error) {
      recordDecision(agent, 'pre-step-error', { message: error instanceof Error ? error.message : String(error) })
      if (!signal.aborted) {
        const message = error instanceof Error ? error.message : String(error)
  ...
  ctx.on('agent/created', ({ agent }) => {
    recordDecision(agent, 'agent-created')
    void Promise.resolve().then(async () => {
      const compaction = await resolveCompaction(ctx, agent, warned.fallbackEngines, warned.fallbackEngineClass)
      recordDecision(agent, 'idle-backend', {
        resolved: compaction !== undefined && typeof compaction.compactNow === 'function',
        fallbackMounted: warned.fallbackEngines.has(agent),
      })
      if (compaction === undefined || typeof compaction.compactNow !== 'function') return
      try {
        measureForDecision(agent, 'idle-measure')
        await enforceIdleThreshold(ctx, compaction, agent, config, warned)
      } catch (error) {
        recordDecision(agent, 'idle-error', { message: error instanceof Error ? error.message : String(error) })
        const message = error instanceof Error ? error.message : String(error)
        ctx.logger.warn(`dsh-auto-compact: idle compaction failed (${message})`)
      }
    })
  })
}
