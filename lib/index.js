 ...
 * agent it resolves that agent's own `ctx.compaction` instance through
 * `agent.ctx.get('compaction')`, so it works no matter which agent preset
 * mounted the compaction backend — standard, minimal, locally authored
 * presets, resumed sessions, and subagents all share the same threshold
 * policy.
 *
 * This file intentionally has NO runtime imports, so it can be installed as a
 * linked local package; all service access goes through the Cordis context.
 */
